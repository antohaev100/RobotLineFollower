 ...
 */
double MeUltrasonicSensor::distanceCmFast(uint16_t MAXcm)
{
  long distance = measureFast(MAXcm * 70);
  ...
  {
    double distanceCm = (double)distance / 58.0;
    return (distanceCm >= MAXcm) ? (double)MAXcm : distanceCm;
  }
}
 ...
 */
long MeUltrasonicSensor::measureFast(unsigned long timeout)
{
  // Direct measurement - no rate limiting, no caching, no validation
  MePort::dWrite2(LOW);
  delayMicroseconds(2);
  MePort::dWrite2(HIGH);
  delayMicroseconds(10);
  ...
  return pulseIn(s2, HIGH, timeout);
}